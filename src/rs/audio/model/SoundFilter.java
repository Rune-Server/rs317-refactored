...
public class SoundFilter {
    ...
    public void method545(Buffer s, boolean flag, SoundEnvelope class29) {
        ...
        if (i != 0) {
            anIntArray668[0] = s.get_ushort();
            anIntArray668[1] = s.get_ushort();
            int j = s.get_ubyte();
            for (int k = 0; k < 2; k++) {
                for (int l = 0; l < anIntArray665[k]; l++) {
                    anIntArrayArrayArray666[k][0][l] = s.get_ushort();
                    anIntArrayArrayArray667[k][0][l] = s.get_ushort();
                }
            ...
            for (int i1 = 0; i1 < 2; i1++) {
                for (int j1 = 0; j1 < anIntArray665[i1]; j1++) {
                    if ((j & 1 << i1 * 4 << j1) != 0) {
                        anIntArrayArrayArray666[i1][1][j1] = s.get_ushort();
                        anIntArrayArrayArray667[i1][1][j1] = s.get_ushort();
                    } else {
