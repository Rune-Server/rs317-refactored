package rs.bzip2;

public class BZip2Context {

    public static int anIntArray587[];
    public boolean aBoolean575;
    public boolean aBooleanArray589[];
    public boolean aBooleanArray590[];
    public byte aByte573;
    public byte aByteArray563[];
    public byte aByteArray568[];
    public byte aByteArray591[];
    public byte aByteArray592[];
    public byte aByteArray594[];
    public byte aByteArray595[];
    public byte aByteArrayArray596[][];
    public final int anInt554 = 4096;
    public final int anInt555 = 16;
    public final int anInt556 = 258;
    public final int anInt557 = 23;
    public final int anInt558 = 1;
    public final int anInt559 = 6;
    public final int anInt560 = 50;
    public final int anInt561 = 4;
    public final int anInt562 = 18002;
    public int anInt564;
    public int anInt565;
    public int anInt566;
    public int anInt567;
    public int anInt569;
    public int anInt570;
    public int anInt571;
    public int anInt572;
    public int anInt574;
    public int anInt576;
    public int anInt577;
    public int anInt578;
    public int anInt579;
    public int anInt580;
    public int anInt581;
    public int anInt582;
    public int anInt584;
    public int anInt588;
    public int anInt601;
    public int anIntArray583[];
    public int anIntArray585[];
    public int anIntArray586[];
    public int anIntArray593[];
    public int anIntArray600[];
    public int anIntArrayArray597[][];
    public int anIntArrayArray598[][];
    public int anIntArrayArray599[][];

    public BZip2Context() {
        anIntArray583 = new int[256];
        anIntArray585 = new int[257];
        anIntArray586 = new int[257];
        aBooleanArray589 = new boolean[256];
        aBooleanArray590 = new boolean[16];
        aByteArray591 = new byte[256];
        aByteArray592 = new byte[4096];
        anIntArray593 = new int[16];
        aByteArray594 = new byte[18002];
        aByteArray595 = new byte[18002];
        aByteArrayArray596 = new byte[6][258];
        anIntArrayArray597 = new int[6][258];
        anIntArrayArray598 = new int[6][258];
        anIntArrayArray599 = new int[6][258];
        anIntArray600 = new int[6];
    }
}
